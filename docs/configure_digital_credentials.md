...
- xPro then asynchronously sends a notification email to the learner with a link to /oauth2/authenticate
  - This uses a redirect url which is a deep link for the wallet app
- The learner logs in, authorizes the OAuth2 request, and is then redirected to another deep link into the wallet app
- This deep link contains a short term auth token which the wallet app exchanges for an access token
